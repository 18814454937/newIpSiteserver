...
with open(homepageData_path, "r+", encoding='utf-8_sig')as f:
...
class TestHomepage(unittest.TestCase):
    def setUp(self):
        ...
        # 调用模块初始化函数并传入配置文件
        Homepage.initialize(self, userConfig)
        self.driver=self.driver



    def test_homepage_01001(self):
        # 获取函数名和截图路径
        ...
        # 调用用例并传入测试数据
        try:
            result = Homepage.homepage_01001(self,homepageData['test_homepage_01001'])
    ...
    def test_homepage_01002(self):
        ...
        # 调用用例并传入测试数据
        try:
            result = Homepage.homepage_01002(self,homepageData['test_homepage_01002'])
        except Exception as e:
            self.driver.get_screenshot_as_file(screenshotPath.format(case_name))
            raise (e)
    ...
    def test_homepage_01003(self):
        ...
        # 调用用例并传入测试数据
        try:
            result = Homepage.homepage_01003(self,homepageData['test_homepage_01003'])
        except Exception as e:
            self.driver.get_screenshot_as_file(screenshotPath.format(case_name))
            raise (e)
